...
func TestGetTemplate(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/authz/templates/read-clusters", nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
	...
	if tmpl.Name != "read-clusters" {
		t.Errorf("expected name 'read-clusters', got %q", tmpl.Name)
	}
}
...
func TestCreateAndListAttachments(t *testing.T) {
	...
	body := `{"template_name":"read-clusters","user_id":"alice"}`
...
func TestCreateAttachment_MissingFields(t *testing.T) {
	...
	body := `{"template_name":"read-clusters"}`
	req := httptest.NewRequest("POST", "/authz/namespaces/my-project/attachments", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
